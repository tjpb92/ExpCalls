...
## Utilisation:
```
java ExpCalls [-dbserver db] -u unum [[-b début] [-e fin]|[-n nbJour]] [-o fichier.xml] 
[-p path] [-s suffixe] [-openedTicket] [-provider tnum] [-agencyId a6num]
[-d] [-t] 
```
où :
...
* ```-u unum``` est la référence au service d'urgence (paramètre obligatoire).
...
* ```-openedTicket``` filtre les tickets ouverts (non clôturés). Désactivé par défaut (paramètre optionel).
* ```-provider tnum``` est la référence à l'intervenant servant à filtrer les tickets. Désactivé par défaut (paramètre optionel).
* ```-agencyId a6num``` est la référence à l'agence servant à filtrer les tickets. Désactivé par défaut (paramètre optionel).
